...
class AsyncEffectTest {
    ...
    private val programConfig =
        ProgramConfig<TestEffect, TestMessage, ProgramModel, Nothing>(
            init = { _ -> Pair(ProgramModel(), emptyList()) },
            update = { message, model ->
                when (message) {
                    is TestMessage.EffectCompleted ->
                        Pair(
                            model.copy(
    ...
    private fun createRunnerConfig(asyncCompletions: Map<String, CompletableDeferred<Unit>> = emptyMap()) =
    ...
    @Test
    fun `sync effect completes immediately and message is queued`() =
        runTest {
            val runnerConfig = createRunnerConfig()

            val programWithEffect =
                programConfig.copy(
    ...
    @Test
    fun `async effect immediate model update applies`() =
        runTest {
    ...
    @Test
    fun `async effect completion function is given the current model not a stale model`() =
        runTest {
            val asyncCompletion = CompletableDeferred<Unit>()
            val runnerConfig = createRunnerConfig(mapOf("async1" to asyncCompletion))
    ...
    @Test
    fun `multiple async effects completing in different order`() =
        runTest {
    ...
    @Test
    fun `mixed sync and async effects`() =
        runTest {
            val asyncCompletion = CompletableDeferred<Unit>()
            val runnerConfig = createRunnerConfig(mapOf("async1" to asyncCompletion))
