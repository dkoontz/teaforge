...
sealed interface Result<out TSuccess, out TError> {
    data class Success<TSuccess, TError>(
        val value: TSuccess,
    ) : Result<TSuccess, TError>

    data class Error<TSuccess, TError>(
        val value: TError,
    ) : Result<TSuccess, TError>
}
...
fun <Success, Error, MappedSuccess> Result<Success, Error>.map(fn: (Success) -> MappedSuccess): Result<MappedSuccess, Error> =
    when (this) {
